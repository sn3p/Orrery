...
function observe() {
  ...
  setInterval(() => {
    ...
    if (app?.catalogue || loader?.source) window.catalogMetrics.states.push({ time: performance.now(), jed: app.jed,
      waiting: !!app.catalogWaiting, committed: loader?.committedCount ?? app.catalogue.count, count: app.asteroidsDiscovered,
      slots: loader?.source.slots.active ?? 0 });
  }, 100);
}
...
async function serve(directory, port = 0) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const relative = decodeURIComponent(url.pathname).replace(/^\/Orrery\//, "/");
  ...
  return { server, traffic, url: "http://127.0.0.1:" + server.address().port + "/Orrery/" };
}
  ...
  try {
    ...
    while (true) {
      const values = await Promise.all([session.send("Runtime.getHeapUsage"), page.evaluate(() => {
        ...
        return { now: performance.now(), first: window.catalogMetrics.initialGpuMs,
          committed: loader?.committedCount ?? app?.catalogue?.count, total: loader?.source?.info.counts.discovery_export ?? app?.catalogue?.count, error: app?.statusError };
      })]);
    ...
    while (true) {
      const [heap, state] = await Promise.all([session.send("Runtime.getHeapUsage"), page.evaluate(() => {
        const app = window.catalogTest.app;
        return { time: performance.now(), complete: (app.catalogLoader?.sceneComplete() ?? true)
          && app.asteroidsDiscovered === app.catalogue?.count };
      })]);
    ...
    const data = await page.evaluate(() => {
      ...
      return { ...window.catalogMetrics, measures: performance.getEntriesByType("measure").filter(entry => entry.name.startsWith("catalog:"))
        ...
        dpr: app.effectivePixelRatio, sourceId: app.catalogLoader?.source.sourceId ?? "historical100k",
        catalogId: app.catalogLoader?.source.info.catalog_id ?? "historical100k",
    ...
    return { profile, restoreMs, restoredDataRequests, initialMs: first, playbackEndMs: playbackEnd, lateJumpMs: fullAt - jumpStart, fullAtMs: fullAt,
