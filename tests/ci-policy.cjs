...
function fixtureKeys(selected = selection()) {
  ...
  const keys = ['legacy/contracts', 'legacy/production', 'lazy-preview', 'catalog', 'three', 'unified/rendering'];
  if (selected.includes('ui')) keys.push('unified/production', 'unified/initialization');
  return new Set(keys);
}
