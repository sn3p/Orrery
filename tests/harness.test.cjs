...
test('native discovery preserves coverage and each browser shard partitions it exactly once', async () => {
  ...
  const required = ['historical fixture assets', 'preview entry', 'preview footer loading', 'first-visit introduction', 'main playback and date controls', 'promoted root', 'configured promotion', 'default indexed catalogue', 'development texture lifecycle', 'raw App lifecycle',
