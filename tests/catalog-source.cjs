...
test("trial assembly retains unrelated webpack defines while replacing the trial setting", async t => {
  ...
  await fs.writeFile(entry, "globalThis.catalogDefinitions = [__SAME_PLUGIN_DEFINE__, __OTHER_PLUGIN_DEFINE__, __CATALOG_SELECTION__.mode];");
  ...
  const context = {};
  require("node:vm").runInNewContext(await fs.readFile(path.join(result.output, "assets", (await fs.readdir(path.join(result.output, "assets"))).find(name => name.endsWith(".js"))), "utf8"), context);
