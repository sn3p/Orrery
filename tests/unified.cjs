...
const output = '.context/pr2/contracts';
...
(async () => {
  assert.notEqual(process.env.ORRERY_TEST_APP, 'unified', 'Raw comparison must not alias the legacy reference');
  // No alias: compare the actual legacy class and actual production controller.
  await build('./tests/unified-fixture.js', path.join(output, 'fixture'));
