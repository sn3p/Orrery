...
async function build(output, base) {
  ...
  const latestConfig = path.join(output, "catalog-latest.json");
...
function instrument() {
  ...
  for (const operation of ["bufferData", "bufferSubData"]) {
    ...
    WebGL2RenderingContext.prototype[operation] = function(...args) {
      ...
      const attributes = window.catalogTest?.app.renderer.asteroids?.geometry.attributes ?? {};
      for (const [name, attribute] of Object.entries(attributes)) {
        if (array === attribute.buffer.data) window.catalogProbe.uploads.push({ operation, name, bytes: array.byteLength,
...
async function run(browser, base, output, name) {
  ...
  for (const mode of ["indexed", "whole"]) {
    ...
    try {
      ...
      assert.equal(await page.evaluate(() => window.catalogProbe.uploads.length), 0,
        "An empty initial prefix never uploads the full preallocated asteroid buffers");
      ...
      const packed = await page.evaluate(async () => {
        const { app, prepareOrbits } = window.catalogTest;
        const asteroids = app.renderer.asteroids;
        const response = await fetch(new URL("data/delivery-v1-" + app.catalogLoader.source.sourceId + "/full/catalog.json", location.href));
        const rows = await response.json(), reference = prepareOrbits(rows, asteroids.epoch);
        const pairs = { aBasis: "bases", aElements: "elements", aMeanAnomaly: "meanAnomalies" };
        const match = Object.entries(pairs).every(([name, key]) => asteroids.geometry.getBuffer(name).data
          .every((value, i) => value === reference[key][i]));
        return { match, capacity: asteroids.discoveryDates.length, committed: asteroids.committedCount,
      ...
      if (!await page.locator(".orrery-options-panel").isVisible()) await page.getByRole("button", { name: "Options" }).click();
      ...
      const supported = await page.evaluate(() => !!window.catalogTest.app.renderer.app.renderer.gl.getExtension("WEBGL_lose_context"));
      if (supported) {
        await page.evaluate(() => { window.lossExtension = window.catalogTest.app.renderer.app.renderer.gl.getExtension("WEBGL_lose_context"); lossExtension.loseContext(); });
  ...
  try {
    ...
    const native = await lifecycle.evaluate(async ({ pin, tiesPin, base }) => {
      ...
      return { emptyComplete, oldClosed: source.closed, changedGeneration: app.loadVersion > 1,
    ...
    await lifecycle.evaluate(async ({ pin, base }) => {
      ...
      app.autoRender = true; app.jedDelta = 0; app.requestRender();
  ...
  try {
    await failedShader.addInitScript(() => {
      ...
      WebGL2RenderingContext.prototype.shaderSource = function(shader, source) {
        return original.call(this, shader, source.includes("vec2 orbitPosition")
    ...
    await failedShader.evaluate(() => {
      window.failedDraws = 0;
      const renderer = catalogTest.app.renderer, render = renderer.render;
      renderer.render = function(...args) { failedDraws++; return render.apply(this, args); };
    });
    await failedShader.waitForTimeout(150);
    assert.equal(await failedShader.evaluate(() => failedDraws), 0, "A terminal graphics failure stops recurring draws");
