...
async function run({ browser, name, application = "unified", output: artifactDirectory, step = (_name, action) => action() }) {
  ...
  try {
    ...
    const result = { browser: name, version: browser.version(), ...await step("orbital numerics and lifecycle", () => exercise(page)), phaseUploads: await step("phase uploads", () => exercisePhaseUploads(page)), preparation: await step("preparation", () => exercisePreparation(page)), pixels: await step("marker pixels", () => pixels(page)), instances: await step("instance pixels", () => instancePixels(page)), recovery: await step("context loss and restoration", () => contextRecovery(page)) };
